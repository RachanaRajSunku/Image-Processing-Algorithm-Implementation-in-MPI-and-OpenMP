...
int main(int argc, char *argv[])
{
  ...
  MPI_Status status;
  //printf("Choose mean(0) or median(1) filter");
  //scanf("%d",&choice);
  ...
  if(proc_id == 0){
	...
	for(i=1;i<=num_workers;i++){
		...
		//MPI_Send(&choice,1,MPI_INT,i,master_type,MPI_COMM_WORLD);
	}
	...
	printf("\nElapsed time is %lf",elapsed_time);
  }
  ...
  int recv_type = FROM_MASTER;
	...
	//MPI_Recv(&choice,1,MPI_INT,0,recv_type,MPI_COMM_WORLD,&status);
